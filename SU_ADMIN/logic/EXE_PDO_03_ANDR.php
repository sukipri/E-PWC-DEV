<?php
    //Save Proccesing Pendaftaran Online step 03 Upload systems 
        ...
        if(isset($_POST['simpan_nomor_02'])){
            $no_jkn = @trim($sql_slash($_POST['no_jkn']));
            $no_rujukan = @trim($sql_slash($_POST['no_rujukan']));
...
$cek_jkn = $j_son['response']['rujukan']['peserta']['noKartu'];
    ...
    if($cek_rujukan==$no_rujukan AND $cek_jkn==$no_jkn ){
        echo"<center><font color=green><b>Rujukan Valid, Silahkan 'klik jika data benar' tapi tunggu Tombol aktif dahulu....</b></font></center>";
        //Procccess    
                $up_jkn_01 = $ms_q("$up TPasien set PasienNoKartuJamin='$no_jkn' WHERE PasienNomorRM='$vpss[PasienNomorRM]'");
                $up_rujukan_01 = $ms_q("$up TRawatJalan set JalanNoRujukan='$no_rujukan',AppJenisDaftar='3' WHERE JalanNoReg='$KDREG' ")    ;
                ?>
