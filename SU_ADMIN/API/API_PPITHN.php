...
header("Access-Control-Allow-Headers: Content-Type, Access-Control-Allow-Headers, Authorization, X-Requested-With");
        ...
        #DATA PPI
        $pwc_vppi01_sw = $ms_q("$sl * FROM Citarum.dbo.tPPI WHERE YEAR(Tanggal)=$GET_TAHUN  order by Tanggal desc ");
        $pwc_nr_vppi01_sw = $ms_nr($pwc_vppi01_sw);
            while($pwc_vppi01_sww = $ms_fas($pwc_vppi01_sw)){
                ...
                #DATA INAP
                $pwc_vinap01_sw = $ms_q("$sl InapNoAdmisi,PasienNomorRM FROM TRawatInap WHERE InapNoAdmisi='$pwc_vppi01_sww[NoReg]'");
