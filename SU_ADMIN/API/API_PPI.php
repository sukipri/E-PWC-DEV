...
header("Access-Control-Allow-Headers: Content-Type, Access-Control-Allow-Headers, Authorization, X-Requested-With");
        ...
        if(empty($tanggal_awal) AND empty($tanggal_akhir)){
            $tgl_parameter  = array(
                "response" => "Parameter tanggal kosong",
                "code"=>201
            ); 
        }else{
          
        #DATA PPI
        $pwc_vppi01_sw = $ms_q("$sl * FROM Citarum.dbo.tPPI WHERE Tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' order by Tanggal desc ");
