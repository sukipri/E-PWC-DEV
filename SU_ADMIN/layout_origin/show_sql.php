...
<?php
	if(isset($_POST['simpan'])){
		...
		$email = @$_POST['email'];
		$alamat = @$_POST['alamat'];
  $ms_q("$in data_anggota(kode,nama,email,alamat)values('$kode','$nama','$email','$alamat')");
 echo"<b>Data has been saved";
 }
?>
<?php
		 $result = $ms_q("$call_sel data_anggota");
   // tampilkan data
   while($row = $ms_fas($result)) {
   	 echo "<b> $row[kode] </b>  <i>$row[nama]</i> <u>$row[email]</u> <b>$row[alamat]</b> ";    
     echo "<br />";
   } ?>
</body>
</html>
