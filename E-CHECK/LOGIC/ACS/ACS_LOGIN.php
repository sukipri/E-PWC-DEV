	  ...
	  if(isset($_POST['ec_acs_login'])){ 
			...
			$jawab_us=@trim($SQL_SL($_POST['jawab_us']));
				if($jawab_us!==$tanya_us){
					...
					echo "<script language='javascript'>window.location = 'http://182.253.60.178/E-PWC/E-CHECK/E-CHECK/EC_LOGIN_01.php'</script>";
				}elseif($jawab_us==$tanya_us ){
						
			//include"../sc/conek.php";
			$mdpass = EN_HS_MD5($passuser);
				$dt=$CL_Q("$CL_SL TBUser WHERE namauser='$username' AND passuser='$mdpass' AND akses='31'");
					$bc=$CL_FR($dt);
			if($bc > 0){
				  // session_start(); // di gunakan untuk mengawali perintah session
				   ...
				   $_SESSION['passuser']=$bc[3];
					...
					echo '<META HTTP-EQUIV="Refresh" Content="0; URL=../LOGIC/PG/PG_H_USER.php?IDAKSES01=31">';
					exit();
						 ?>
