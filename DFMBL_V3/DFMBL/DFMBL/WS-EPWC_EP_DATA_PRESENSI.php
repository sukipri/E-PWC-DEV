...
<span class="mx-2"><b>#DATA PRESENSI</b></span>
<div class="input-group input-group-sm mb-3 mx-2">
  <input type="date" class="form-control form-control-sm" required name="txt_cari_data">
  <select name="cb_cari_data" class="form-control form-control-sm" required>
      <option value=>Bagian</option>
      <?PHP 
         $epwc_ctr_vunit01_sw = $CL_Q("$SL UnitKode,UnitNama FROM  Citarum.dbo.TUnitPrs order by UnitNama asc ");
            while($epwc_ctr_vunit01_sww = $CL_FAS($epwc_ctr_vunit01_sw)){
                echo"<option value=$epwc_ctr_vunit01_sww[UnitKode]>$epwc_ctr_vunit01_sww[UnitNama]</option>";
            }
      ?>
  </select>
...
<?PHP 
    if(isset($_POST['btn_cari_data'])){
     $ep_ri_tgl01  = @$_POST['txt_cari_data']; 
     $cb_cari_data  = @$_POST['cb_cari_data'];
     $ep_ri_tgl02 = date("tomorrow");

     $epwc_view02_vkry01_sw = $CL_Q("$SL KaryNomor,KaryNomorYakkum,KaryNama,UnitKode FROM Citarum.dbo.TKaryawan WHERE UnitKode='$cb_cari_data' AND   NOT KaryStatus='91' AND NOT KaryStatus='99'  AND NOT KaryStatus='92'  AND NOT KaryStatus='94'  AND NOT KaryStatus='93' ");
 ...
 ?>
 <span class="badge bg-info mx-2"><?PHP echo $epwc_view02_vkry01_sww['KaryNama']; ?></span>
 <br>
 ...
 <?PHP
             ...
             <i><?PHP echo "Keterlambatan ".$jam." Jam ".$rn_menit02." Menit "; ?></i>
