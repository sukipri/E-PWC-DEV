<?php
	...
	//include"../../CONFIG/MYSQL/MY_CONNECT_PRD.php";
	  if(isset($_POST['epwc_acs_login'])){ 
			...
			$mdpass =EN_HS_CR32($passuser);
				$dt=$CL_Q("$CL_SL TBUser WHERE namauser='$username' AND passuser='$mdpass' AND akses='4'");
					$bc=$CL_FR($dt);
			if($bc > 0){
				  // session_start(); // di gunakan untuk mengawali perintah session
				   ...
				   $_SESSION['passuser']=$bc[3];
					...
					echo '<META HTTP-EQUIV="Refresh" Content="0; URL=../LOGIC/PG/PG_H_USER.php?IDAKSES01=4">';
					exit();
						 ?>
