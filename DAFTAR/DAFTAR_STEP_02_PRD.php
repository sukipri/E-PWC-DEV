...
<html>
  ...
  <li class="breadcrumb-item active">Masukan Nomor RM & Tanggal Pesan *(Pribadi</li>
</ol>
	<form method="post" action="">
	  ...
	  <div class="input-group">
        ...
        <tr>
          <td> 
		  <div class="input-group">
          <div class="input-group-addon">Nomor Reg</div> <!-- PL-1806-1001 -->
	 		<input type="text" name="noreg"  class="form-control" readonly value="<?php if($hit_zero > 9999){ echo"PL-$ON$month$hit_zero";}elseif($hit_zero < 9999){ echo"PL-$month$ON$hit_zero";} ?>" required>
	  </div>
	  <br>
		  <div class="input-group">
  				...
  				<strong>Maaf!!</strong> Nomor Rekam Medis Tidak Ditemukan
						...
						<?php
							if(empty($waktu)){
								...
								<div class="alert alert-dismissible alert-danger">
  			<button type="button" class="close" data-dismiss="alert">&times;</button>
  				<strong>Mohon Maaf</strong> Pengindenan harus 1 hari sebelum periksa
							</div>
							<?php //}elseif($date_web==$dt_tgl){?>
							<!--
							<div class="alert alert-dismissible alert-danger">
  					<button type="button" class="close" data-dismiss="alert">&times;</button>
  					<strong>Maaf!!</strong> Pengindenan harus 1 hari sebelum periksa
							</div>
								-->
								<?php
								}else{
						...
						header("location:DAFTAR_STEP_03_PRD.php?IDDKT=$IDDKT&RM=$rm&REG=$noreg"); 
						 echo"OKEEE";
						  }}}
						?>
