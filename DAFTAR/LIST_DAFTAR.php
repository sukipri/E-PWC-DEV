...
<br><br><br><br>
		<div class="container">
	<ol class="breadcrumb">
	<li class="breadcrumb-item"><i class="fa fa-share"></i>&nbsp;Check Registrasi</li>
  <li class="breadcrumb-item">Pencarian Nomor Registrasi</li>
</ol>
		 <table width="100%" border="0" class="table table-bordered">
          <tr class="danger">
            <td width="2%">#</td>
            <td width="33%">NO RM,Nama</td>
            <td width="24%">Tgl Pesan</td>
            <td width="14%">Tgl Periksa </td>
            <td width="27%">Dokter,Poli</td>
          </tr>
		  	<?php
				if(isset($_POST['cari'])){
					$nama = @trim($_POST['nama']);
			$vps = @$ms_q("$sl TOP 5 * FROM TRawatJalan where PasienNomorRM='$nama' AND JalanCaraDaftar='4' AND NOT AppJenisDaftar='1' order by TanggalPesan desc");
				$jum_vps = $ms_nr($vps);
					...
					$vpl = $ms_q("$sl UnitNama from TUnit where UnitKode='$vpss[UnitKode]'");
						...
						$vdkt =  $ms_q("$sl PelakuKode,PelakuNama from TPelaku where PelakuKode='$vpss[DokterKode]'");
							...
							$vjdll = $ms_fas($vjdl);
						$vrj_con_02= $ms_q("select CONVERT(varchar(10),[JalanRMTanggal],101) as tgl_rjj from TRawatJalan where JalanNoReg='$vpss[JalanNoReg]'");
          ...
          <tr class="success">
            <td><?php echo"$no"; ?></td>
            <td><a href="<?php echo"DAFTAR_STEP_04.php?IDDKT=$vpss[DokterKode]&RM=$vpss[PasienNomorRM]&REG=$vpss[JalanNoReg]&OKEEE=OKEEE#LENGKAP"; ?>"><?php echo"$vpss[PasienNomorRM]"; ?></a><br><?php echo"$vpss[PasienNama]"; ?></td>
            <td><?php echo"$vrjj_con_03[tgl_rjjj]"; ?></td>
            <td><?php echo"$vrjj_con_02[tgl_rjj]"; ?></td>
            <td>
			 <?php 
			 echo"$vdktt[PelakuNama]<br>";
			 echo"$txt_01";
          ...
          <tr>
		  	...
		  	if($vpss['AppJenisDaftar']==3){ 
				echo"<b>Nomor Urut: &nbsp; <i>Menunggu Persetujuan Petugas....</i></b><br><font color=grey>*(Jadwal Pengkonfirmasian dilakukan jam 09-00 AM - 17-00 PM Kecuali Hari Minggu </font>";
					...
					if($vpss['WaktuPesan']=="P"){
						echo"<b>Nomor Urut: &nbsp; <i>$vpss[JalanNoUrut]</i>, Silahkan datang pada tanggal $vrjj_con_02[tgl_rjj] ($vjdll[KetJam]) </b>  <br> $text_jam ";
						echo"<br><i>$vpss[ket]</i>";
            ...
            <td>&nbsp;</td>
            <td>
			<?php if($vpss['AppJenisDaftar']==2){ ?>
		  <a href="<?php echo"DAFTAR_PRINT.php?IDDKT=$vpss[DokterKode]&REG=$vpss[JalanNoReg]&RM=$vpss[PasienNomorRM]#$REG"; ?>" target="_blank" class="btn btn-primary btn-sm"><i class="fa fa-print"></i>&nbsp;Cetak Pendaftaran</a>
		  <?php }else{ ?>
