<h5>DATA VIEW CP FORM PASIEN *Obsgin</h5>
<br>
<form method="post">
...
<?PHP 
    $cpf_cp_no = 1;
    $cpf_inap_vform01_sw = $CL_Q(" $SL DISTINCT idmain_inap_01 FROM  Citarum.dbo.tb_cpf01_form03_01 WHERE form_tglinput_01 BETWEEN '$tg01' AND '$tg02 23:59:00' ");
    while($cpf_inap_vform01_sww = $CL_FAS($cpf_inap_vform01_sw)){
        #DATA FORM
        $cpf_vform01_sw = $CL_Q(" $CL_SL  Citarum.dbo.tb_cpf01_form03_01 WHERE idmain_inap_01='$cpf_inap_vform01_sww[idmain_inap_01]' ");
            $cpf_vform01_sww = $CL_FAS($cpf_vform01_sw);
        #DATA KEG_03
        $cpf_vkeg03_sw = $CL_Q("$SL idmain_keg_03,keg_nama_03 FROM Citarum.dbo.tb_cpf01_keg03_03 WHERE idmain_keg_03='$cpf_vform01_sww[idmain_keg_03]'");
...
<tr>
    ...
    <td>
        <?PHP
             echo "<a href=?PG_SA=CPF01_CP03_01_FORM&IDADM01=$cpf_inap_vform01_sww[idmain_inap_01]&IDRM01=$cpf_vform01_sww[idmain_pasien_01]&IDKEG03=$cpf_vkeg03_sww[idmain_keg_03]>".$cpf_vform01_sww['idmain_inap_01']."</a>";
             echo"<br>";
        ...
        $cpf_cek_vformp01_sw = $CL_Q("$SL idmain_formp_01,formp_tot_01 FROM Citarum.dbo.tb_cpf01_form03_01_head_persen WHERE idmain_inap_01='$cpf_inap_vform01_sww[idmain_inap_01]'");
    ...
    <td>
        <?PHP 
    #KALKULASI
    $cpf_data_vform01_sw = $CL_Q(" $SL DISTINCT idmain_keg_01 FROM   Citarum.dbo.tb_cpf01_form03_01 WHERE idmain_inap_01='$cpf_inap_vform01_sww[idmain_inap_01]' ");
        while($cpf_data_vform01_sww = $CL_FAS($cpf_data_vform01_sw)){
            #DATA KEGIATAN
            $cpf_nr_vkeg03rec_sw = $CL_Q("$SL idmain_keg_03_rec,idmain_keg_02,idmain_keg_03,idmain_keg_01 FROM Citarum.dbo.tb_cpf01_keg03_03_rec WHERE idmain_keg_03='$cpf_vkeg03_sww[idmain_keg_03]' AND idmain_keg_01='$cpf_data_vform01_sww[idmain_keg_01]'");
            $cpf_nr_vkeg03rec_sww = $CL_NR($cpf_nr_vkeg03rec_sw);
            #DATA PENUNJANG
            $cpf_vkeg01_sw = $CL_Q("$SL idmain_keg_01,keg_nama_01 FROM Citarum.dbo.tb_cpf01_keg03_01 WHERE idmain_keg_01='$cpf_data_vform01_sww[idmain_keg_01]' order by keg_urut_01 asc");  
            $cpf_vkeg01_sww = $CL_FAS($cpf_vkeg01_sw);

            //echo $cpf_vform01_sww['idmain_keg_01'];
            #KAL Pem.Penunjang
            $cpf_nr_1_vform01_sw = $CL_Q("$SL SUM(form_nilai02_01) as tot_nilai02 FROM Citarum.dbo.tb_cpf01_form03_01  WHERE form_nilai02_01='1' AND idmain_inap_01='$cpf_inap_vform01_sww[idmain_inap_01]' AND idmain_keg_01='$cpf_vkeg01_sww[idmain_keg_01]'");
