<?PHP echo"<h5>NO ADMISI $IDADM01 #FORM CPF $cpf_vw_vkeg03_sww[keg_nama_03] - Denomerator $cpf_vw_vkeg03_sww[keg_rawat_03] </h5>"; #TITLE
        
        #DATA VALIDATION
        $cpf_val_vform01_sw = $CL_Q("$SL idmain_cpf01_form_01,idmain_pasien_01,idmain_inap_01,idmain_keg_03 FROM Citarum.dbo.tb_cpf01_form_01 WHERE idmain_pasien_01='$IDRM01' AND idmain_inap_01='$IDADM01'AND idmain_keg_03='$IDKEG03'");
...
<?PHP 
    $cpf_keg01_no = 1;
    $cpf_vkeg01_sw = $CL_Q("$CL_SL Citarum.dbo.tb_cpf01_keg_01 order by keg_urut_01 asc ");
...
<?PHP 
    $cpf_cp_no = 1;
    $cpf_vkeg03rec_sw = $CL_Q("$CL_SL Citarum.dbo.tb_cpf01_keg_03_rec WHERE idmain_keg_03='$cpf_vw_vkeg03_sww[idmain_keg_03]' AND idmain_keg_01='$cpf_vkeg01_sww[idmain_keg_01]'"); #MAIN DATA VIEW REC
    $cpf_nr02_vkeg03rec_sww = $CL_NR($cpf_vkeg03rec_sw);
    
     while($cpf_vkeg03rec_sww = $CL_FA($cpf_vkeg03rec_sw)){
        $cpf_vkeg02_sw = $CL_Q("$SL idmain_keg_02,idmain_keg_01,keg_nama_02,keg_ket_02,keg_status_02 FROM Citarum.dbo.tb_cpf01_keg_02 WHERE idmain_keg_02='$cpf_vkeg03rec_sww[idmain_keg_02]'");#DATA KEG_02
            $cpf_vkeg02_sww = $CL_FAS($cpf_vkeg02_sw);
          #DATA FORM 01
			$cpf_vform01_sw = $CL_Q("$CL_SL Citarum.dbo.tb_cpf01_form_01 WHERE idmain_pasien_01='$IDRM01' AND idmain_inap_01='$IDADM01'AND idmain_keg_03='$IDKEG03' AND idmain_keg_03_rec='$cpf_vkeg03rec_sww[idmain_keg_03_rec]'");
...
<?PHP $cpf_cp_no++; } 
    $cpf_sum_vform01_sw = $CL_Q("$SL SUM(form_nilai_01) as sum_nilai FROM Citarum.dbo.tb_cpf01_form_01 WHERE idmain_pasien_01='$IDRM01' AND idmain_inap_01='$IDADM01'AND idmain_keg_03='$IDKEG03' AND idmain_keg_01='$cpf_vkeg01_sww[idmain_keg_01]' ");
    $cpf_sum_vform01_sww = $CL_FAS($cpf_sum_vform01_sw);
    $cpf_nr_1_vform01_sw = $CL_Q("$SL SUM(form_nilai02_01) as tot_nilai02 FROM Citarum.dbo.tb_cpf01_form_01  WHERE form_nilai02_01='1' AND idmain_pasien_01='$IDRM01' AND  idmain_inap_01='$IDADM01' AND idmain_keg_01='$cpf_vkeg01_sww[idmain_keg_01]'");
...
<?PHP  $cpf_keg01_no++;}  
    ...
    $cpf_nr_vkeg03rec_sw = $CL_Q("$SL idmain_keg_03_rec,idmain_keg_03 FROM Citarum.dbo.tb_cpf01_keg_03_rec WHERE idmain_keg_03='$cpf_vw_vkeg03_sww[idmain_keg_03]'"); #HIT NUMBER
...
<?PHP 
     if($cpf_nr_val_vform01_sww > 0){
?>
    <button class="btn btn-warning" name="cp_up_01"> 1.UPDATE DATA</button>
&nbsp
    <button class="btn btn-dark" name="cp_upload_01"><i class="fas fa-calculator"></i> 2.KALKULASI DENOM</button>
&nbsp
    <?PHP
        #DATA ROW F HEAD
        $cpf_nr_vfhead01_sw = $CL_Q("$SL idmain_cpf01_form_01_head,idmain_inap_01 FROM Citarum.dbo.tb_cpf01_form_01_head WHERE idmain_inap_01='$IDADM01'");
            $cpf_nr_vfhead01_sww = $CL_NR($cpf_nr_vfhead01_sw);
    ?>
    <?PHP if($cpf_nr_vfhead01_sww > 0 ){ ?>
    <button class="btn btn-info" name="cp_tutup02_01"><i class="fas fa-toggle-off"></i> 3.TUTUP FORM</button>
    <?PHP }elseif($cpf_nr_vfhead01_sww < 1){ ?>
        <button class="btn btn-secondary" name="cp_tutup_01"><i class="fas fa-toggle-off"></i> 3.TUTUP FORM</button>
    <?PHP } ?>
<?PHP }else{ ?>
    <button class="btn btn-success" name="cp_simpan_01"> 1.SIMPAN DATA</button>
<?PHP } ?>
</form>
<br>
