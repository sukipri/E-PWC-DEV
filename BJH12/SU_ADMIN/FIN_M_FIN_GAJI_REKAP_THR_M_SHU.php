        ...
        <form method="post" action="#SUKSES">
          ...
          <table width="100%" class="table table-bordered" border="0" style="max-width:70rem;">
            ...
            <tr>
              ...
              <td>
                <?php
					$vgj01_sw = $ms_q("$sl GajiTahun,GajiTHRJenis,KaryNomor,KaryNama,GajiTotal,GajiPph21,GajiBersih,GajiStatus,GajiOtorisasi,GajiDibayarkan FROM TGajiTHR WHERE KaryNomor='$vkryy_up[KaryNomor]' AND GajiTHRJenis='B' AND GajiStatus='1' AND GajiTahun='$YR' ");
             ...
             <table width="100%" border="0" class="table table-bordered table-sm">
                	...
                	<tr>
                      ...
                      <td><?php echo"Rp.$nom_bersih_01"; ?></td>
                        ...
                        <a class="badge badge-success" href="#">Terverif</a>
                        <?php }elseif($vgj01_sww['GajiOtorisasi']=="0"){ ?>
                          <a class="badge badge-danger" href="#">Belum Terverif</a>
                        <?php } ?>
		...
		<?php } ?>
          ...
          <!-- -->
       	<nav class="navbar fixed-bottom navbar-light bg-light">
           	...
           	<div class="txt_right">
          <?php
		  	$vgj_jum_acc = $ms_q("$call_sel TGajiTHR WHERE   GajiOtorisasi='1' OR  GajiOtorisasi='4' AND GajiTahun='$YR' AND GajiTHRJenis='B' order by GajiTahun desc");
				$vgjj_jum_acc = $ms_nr($vgj_jum_acc);
			$vgj_jum_bacc = $ms_q("$call_sel TGajiTHR WHERE   GajiOtorisasi='0' AND GajiTahun='$YR' AND GajiTHRJenis='B' order by GajiTahun desc");
					...
					$no_gaji = 1; 
						while( $no_gaji <= $jum_vkry){
							...
							$nik = @$_POST['nik'.$no_gaji];
								$ms_q("$up TGajiTHR set GajiOtorisasi='$pilih' WHERE KaryNomor='$nik' AND GajiTahun='$gaji' AND GajiTHRJenis='B'");
								//echo"$pilih  - $nik<br>";
						$no_gaji++;}
			?>
...
</div>      <meta http-equiv="refresh" content="0; url=?HLM=FIN_M&SUB=FIN_M_FIN_GAJI_REKAP_THR_M&SUB_CHILD=FIN_M_FIN_GAJI_REKAP_THR_M_SHU">
            <?php } ?>
        </form>
</body>
<?php } ?>