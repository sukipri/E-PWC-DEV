		...
		if(isset($_POST['simpan_berkas'])){
			...
			if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
				if($ukuran < 9044070){			
					...
					$ms_q("$up TRawatJalan set AppKhusus='2' where JalanNoReg='$REG'"); 
					if($query){
						echo 'Failed';
					}else{
						//echo "Uploaded";
	...
	<form method="post" enctype="multipart/form-data">
	   ...
	   <label for="email">No pendaftaran</label>
		  ...
		  //
		  	$vgbr_01 = $ms_q("$sl JalanNoReg,GambarStatus from TBGambar where JalanNoReg='$REG' AND GambarStatus='1'");
				$hit_vgbr_01 = $ms_nr($vgbr_01);
					if($hit_vgbr_01 < 1){
		   ?>
		    <label>Unggah kartu BPJSK *Unggahan wajib kkk</label>
           ...
           <tr class="txt">
			 ...
			 <tr class="txt">
              <th>
				...
				<?php 
		  //
		  	$vgbr_02 = $ms_q("$sl JalanNoReg,GambarStatus from TBGambar where JalanNoReg='$REG' AND GambarStatus='2'");
				$hit_vgbr_02 = $ms_nr($vgbr_02);
					if($hit_vgbr_02 < 4){
		   ?>
		       <label>Unggah surat rujukan *Unggahan wajib</label>
		...
		<input type="file" name="up"   accept="image/jpeg" disabled>
           ...
           <tr class="txt">
              <th align="right"> <button name="simpan_rujukan" class="waves-effect waves-light btn small"><i class="material-icons left">file_upload</i> Unggah</button></th>
		    </tr>
			 <tr class="txt">
              <th>
			    <label>Unggah surat kontrol/Trauma/Konsul *Unggahan pilihan / tidak wajib</label>
		 <input type="file" name="up_berkas"   accept="image/jpeg"></th>
		    </tr>
           <tr class="txt">
              <th align="right"> <button name="simpan_berkas" class="waves-effect waves-light btn small"><i class="material-icons left">file_upload</i> Unggah</button></th>
		    </tr>
       </thead>
	   </table>
	   <table class="table striped">
	   <?php
				...
				?>
					...
					echo "<td>Kartu BPJSK <a href=UPLOAD_BERKAS.php?IDDKT=$IDDKT&RM=$RM&REG=$REG&IDG=$vgbrr_02[idmain_gambar]#DELETE class=txt><b>X Hapus Gambar</a></td>";
						echo"</tr>";
						echo" <tr>";
